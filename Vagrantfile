...
Vagrant.configure("2") do |config|
    config.vm.hostname = "vmustache.local"
    config.vm.network :private_network, ip: "33.33.33.89"

    config.vm.provider :virtualbox do |vb|
        config.vm.box = "ubuntu/trusty64"

        vb.name = "vmustache VIM Tests"
    ...
    config.vm.synced_folder "./", "/home/vagrant/vmustache", :nfs => (RUBY_PLATFORM =~ /linux/ or RUBY_PLATFORM =~ /darwin/)
end
